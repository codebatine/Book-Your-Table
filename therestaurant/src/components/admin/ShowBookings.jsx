...
import { editBooking, removeBooking, getBookings, loadReadContract } from '../../services/blockchainService.js';
...
export const ShowBookings = ({ restaurantId, all }) => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        if (!readContract) {
          console.log('Read contract not initialized. Loading...');
          const contract = await loadReadContract();
  ...
  const handleEdit = async (bookingId) => {
    ...
    try {
      ...
      await editBooking(bookingId, numberOfGuests, name, date, time);
